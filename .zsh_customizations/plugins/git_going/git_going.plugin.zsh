# copied from grb prompt theme (Gary Bernhardt) and modified
...
function prompt_git_going_scm_time_since_commit() {
	local -A pc
...
function prompt_git_going_scm_info() {
    if zgit_inworktree; then
        echo "($(prompt_git_going_scm_time_since_commit)|$(prompt_wunjo_scm_branch))"
    fi
}

prompt_git_going_setup() {
    local verbose
...
#"
    ...
    PROMPT+="\$(prompt_git_going_scm_info)"
	PROMPT+=" $pc[#]\$$pc[reset] "
