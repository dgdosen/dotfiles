...
build_prompt() {
  ...
  prompt_context
  prompt_dir
  prompt_grb_scm_time_since_commit
  prompt_git
  prompt_hg
  prompt_end
}
...
# hack for time since last commit
#
...
function prompt_grb_scm_time_since_commit() {
	...
	if zgit_inworktree; then
        ...
        if [ "$MINUTES_SINCE_LAST_COMMIT" -eq -1 ]; then
          ...
          UNIT="m"
          local SINCE_LAST_COMMIT="uncommitted$pc"
        else
        ...
        # local gray = "\033[48;5;250"
        prompt_segment black $COLOR  $SINCE_LAST_COMMIT
